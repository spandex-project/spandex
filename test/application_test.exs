defmodule Spandex.ApplicationTest do
  ...
  defmodule TestBroadcast do
    ...
    def put(_, body, _) do
      send(
        Spandex.ApplicationTest,
        {:sent_spans_to_dd, body |> Msgpax.unpack!() |> hd() |> Enum.count()}
      )
  ...
  test "correctly starts supervisor" do
    ...
    with_conf(:spandex, :log_traces?, true, fn ->
      with_conf(:spandex, :datadog, conf, fn ->
        ...
        assert response =~
                 ~r/Trace response: {:ok, %HTTPoison.Response{body: \"OK\", headers: \[\], request_url: nil, status_code: 200}}/
      end)
    end)
